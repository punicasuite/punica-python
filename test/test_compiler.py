...
class TestCompiler(unittest.TestCase):
    def setUp(self):
        pj_path = os.path.join(os.getcwd(), 'file', 'compile')
    ...
    def test_compile_contract(self):
        contract_path = os.path.join(os.getcwd(), 'file', 'compile', 'build', 'oep4.py')
        ...
        with open(os.path.join(os.getcwd(), 'file', 'compile', 'oep4.avm'), 'r') as f:
            target_avm = f.read()
        with open(os.path.join(os.getcwd(), 'file', 'compile', 'oep4.json'), 'r') as f:
            target_abi = f.read()
    ...
    def test_generate_avm_file(self):
        contract_path = os.path.join(os.getcwd(), 'file', 'compile', 'oep4.py')
        split_path = os.path.split(contract_path)
        save_path = os.path.join(os.getcwd(), 'build', split_path[1])
        avm_save_path = save_path.replace('.py', '.avm')
    ...
    def test_generate_avm_code(self):
        path = os.path.join(os.getcwd(), 'file', 'compile', 'oep4.py')
        hex_avm = PunicaCompiler.generate_avm_code(path)
        with open(os.path.join(os.getcwd(), 'file', 'compile', 'oep4.avm'), 'r') as f:
            self.assertEqual(f.read(), hex_avm)

    def test_generate_invoke_config(self):
        abi_path = os.path.join(os.getcwd(), 'file', 'compile', 'oep4_token_abi.json')
        invoke_config_path = os.path.join(os.getcwd(), 'file', 'compile', 'invoke_config.json')
